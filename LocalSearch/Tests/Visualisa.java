...
public static String getMap(String s) {
		...
		if(s.equals("10")) return "maps/newMaps/map0.xml";
		if(s.equals("11")) return "maps/newMaps/basesWorkers24x24A.xml";
		return null;
	}
...
public static double partida(GameState gs,UnitTypeTable utt, int player, int max_cycle, AI ai1, AI ai2, boolean exibe) throws Exception {
        ...
        do {
        	...
        	try {
     			...
     			int t =gs2.getTime();
             	for (Unit uuu : pgs.getUnits()) {
             		if(uuu.getPlayer()==1) {
             			UnitActionAssignment uaa = gs2.getActionAssignment(uuu);
             			if (uaa != null) {
                ...
                for ( Pair<Unit,UnitAction> ua : pa1.getActions()) {
                	List<PlayerAction> paa = gs2.getPlayerActionsSingleUnit(ua.m_a);
                	for(PlayerAction aa : paa) {
                		for ( Pair<Unit, UnitAction> aux: aa.getActions()) {
                			if( aux.m_b.getType()==5 ) {
                				int ttt = t-gs2.getTime();
                				System.out.println(gs2.getTime() +" "+aux+"   "+ua+"  "+ttt);
                        	}		
                		}
                	}	
                }
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("foi");		//AI ai1 = new Interpreter(utt,n);
		AI ai1 = new NaiveMCTS(100, -1, 100,10,0.3f, 1.0f, 0.0f, 1.0f, 0.4f, 1.0f, new RandomBiasedAI(), new SimpleSqrtEvaluationFunction3(),false);	
		...
		for(int i =0;i<1;i++) {
			...
			partida(gs2,utt, 0, 6000, ai1, ai2, true);
