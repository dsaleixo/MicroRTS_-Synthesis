...
public class CS_Origin implements Avaliador {
    ...
    public CS_Origin() {
        ...
        System.out.println("Camp\t" + 0.0 + "\t" + 0 + "\t"+adv_atual.size()+"\t"
    ...
    public double Avalia(GameState gs, int max, Node_LS n) throws Exception {
        ...
        double r = 0;
        double rt0 = playout.run(gs, 0, max, ai, adv_atual.get(adv_atual.size() - 1), false).m_a;
        double rt1 = playout.run(gs, 1, max, ai, adv_atual.get(adv_atual.size() - 1), false).m_a;
...
//        }
    ...
    private List get_stronger(GameState gs, int max, Node_LS strategy, List<Node_LS> opponents) throws Exception {
        ...
        for (Node_LS opponent : opponents) {
            ...
            double r0 = playout.run(gs, 0, max, ai2, ai, false).m_a;
            double r1 = playout.run(gs, 1, max, ai2, ai, false).m_a;
            if ((r0 + r1) > 1.0) {
