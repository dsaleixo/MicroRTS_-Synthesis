...
public class SimplePlayout implements Playout {
	...
	public Pair<Double, CabocoDagua2> run(GameState gs, int player, int max_cycle, AI ai1, AI ai2, boolean exibe) throws Exception {
		...
		int cont_atraso=0;
		int lim =1000;
		if (max_cycle> 10000)lim=3000;
		CutOffLTD2 cutOof = new CutOffLTD2(lim);
        do {
        	...
        	try {
        		...
        		if(paraou>100) {
        			System.out.println("quebrou " +cont_atraso );
        		...
        		if(paraou>75) {
        			...
        			cont_atraso+=1;
        			if(cont_atraso>10) {
        				System.out.println("quebro2 " +cont_atraso );
        				return new Pair<>(-1.0,new CabocoDagua2());
        ...
        } while (!gameover && (gs2.getTime() <= max_cycle) &&  !cutOof.stop(gs2)); 
