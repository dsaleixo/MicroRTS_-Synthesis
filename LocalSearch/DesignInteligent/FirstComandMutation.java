...
public class FirstComandMutation implements Mutation {

	public FirstComandMutation() {
	...
	public List<Node_LS> mutation(Node_LS node, int n) {
		...
		;	if(selects2.size()==0)return individuos;
			For_S_LS fs =(For_S_LS) selects2.get(0);
			
			C_LS c = new C_LS();
			c.mutation(0, 15, false);
			S_LS s = new S_LS(c);
			S_S_LS ss = new S_S_LS(s,fs.getChild());
			S_LS s2 = new  S_LS(ss);
			fs.setChild(s2);
			individuos.add(n2);
		}
	...
	public static void main(String[] args) {
		FirstComandMutation m =new FirstComandMutation();
