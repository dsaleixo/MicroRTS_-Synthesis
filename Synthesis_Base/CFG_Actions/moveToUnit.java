...
public class moveToUnit implements ChildC {
	...
	public void interpret(GameState gs, int player, Unit u, Interpreter automato) throws Exception {
		...
		PhysicalGameState pgs = gs.getPhysicalGameState();
		if (automato.me.containblockTable(this.getName()+u.getID()+OP.translate()+targetplayer.translate())){
		...
		if(u.getType().canMove && u.getPlayer()==player && automato.getAbstractAction(u)==null ) {
			...
			if(u2!=null) {
				...
				Pair<Integer,Integer> move = pf.findPathToPositionInRange2(u, u2.getX() + u2.getY() * pgs.getWidth(),1,
						gs,automato.me);
				...
				if(move!=null) {
					...
					automato.me.setblockTable(this.getName()+u.getID()+OP.translate()+targetplayer.translate());
