...
public class AStarPathFindingForDSL extends PathFinding {
...
//	                    System.out.println("    " + pos%w + "," + pos/w);
	    ...
	    public Pair<Integer, Integer> findPathToPositionInRange2(Unit start, int targetpos, int range, GameState gs, Memorization me) {
	        PhysicalGameState pgs = gs.getPhysicalGameState();
	        int w = pgs.getWidth();
	        int h = pgs.getHeight();
	        if (free==null || free.length<w*h) {
...
//	        System.out.println("Looking for path from: " + start.getX() + "," + start.getY() + " to " + targetx + "," + targety);
	        ...
	        int cont=0;
	        while(openinsert>0) {
	        	//if(cont>3*manhattanDistance(start.getX(), start.getY(), targetx, targety)) {
	        	//	break;
	        	//}
	        	//cont++;
	            // debugging code:
	            ...
	            */
	            ...
	            if(!me.thereIsPath(x, y, targetx, targety)) {
...
//	                    System.out.println("    " + pos%w + "," + pos/w);
	        ...
	        me.setNoPath(visited, targetx, targety);
	        return null;
	    }
