...
public class HasUnitWithinDistanceFromOpponent implements ChildB {
	...
	public void interpret(GameState gs, int player, Unit u, Interpreter automato) {
		// TODO Auto-generated method stub
		 ...
		 if(automato.me.ContainsBool(this.getName()+distance)) {
				this.value = automato.me.getBool(this.getName()+distance);
	    		return;
	    		
	    	}
		 
		 automato.me.setBool(this.getName()+distance, false);
		 this.value=false;
		 ...
		 for(Unit u2:pgs.getUnits()) {
			...
			// if(u.getType()==UTType)System.out.println("d");
	            if (  u2.getPlayer() == player_enemy  ) {
                   ...
                   if (d<=distance) {
                	   automato.me.setBool(this.getName()+distance, true);
                   		this.value=true;
                   }
